...
class wpu_settings_version {
    ...
    public function activate_plugins($plugins = array(), $network_wide = false, $silent = false) {
        if (!is_array($plugins)) {
            return false;
        }
        ...
        foreach ($plugins as $plugin) {
            if (!is_plugin_active($plugin)) {
                activate_plugin($plugin, '', $network_wide, $silent);
            }
        }
    }
    ...
    /* ----------------------------------------------------------
      Add args to an option
    ---------------------------------------------------------- */

    function add_args_to_option($option_id, $args = array()) {
        $opt = get_option($option_id);
        if (!is_array($opt)) {
            $opt = array();
        }
        $opt = array_merge($opt, $args);
        update_option($option_id, $opt);
    }
